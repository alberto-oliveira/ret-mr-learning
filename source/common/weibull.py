...
class WeibullMR(BaseEstimator):
    ...
    @staticmethod
    def weibull_quant_r(shp, scl, p):

        t = ExtDist.qWeibull(p, shape=shp, scale=scl)
        sys.stderr.flush()

        return t[0]

    @staticmethod
    def weibull_estim_r(data):

        data_r = robjects.FloatVector(data.reshape(-1).tolist())
        estpar = ExtDist.eWeibull(data_r, method="numerical.MLE")
        sys.stderr.flush()
